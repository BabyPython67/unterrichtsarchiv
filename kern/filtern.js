...
/** Nach Kurs (sortiert nach Anzeigename), darin chronologisch mit Monatsblöcken. */
export function gruppieren(eintraege, kursAlias = {}) {
  ...
  return kurseSortiert(nachKurs.keys(), kursAlias).map((kurs) => {
    const liste = [...nachKurs.get(kurs)].sort((a, b) =>
      a.datum.localeCompare(b.datum) || a.position - b.position);
    const monate = [];
    ...
    return {
      ...
      von: liste[0].datum, bis: liste[liste.length - 1].datum, monate,
    };
  });
}
